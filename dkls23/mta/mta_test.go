...
func TestMultiplicationMultipleRuns(t *testing.T) {
	// Run the test multiple times to ensure consistency
	for run := 0; run < 3; run++ {
		t.Run("run", func(t *testing.T) {
			sessionID := []byte("test-multiplication-multi")

			// Initialize
			otSender, dlogProof, nonce, _ := InitReceiverPhase1(sessionID)
			otReceiver, correlation, vecR, encProofs, _ := InitSenderPhase1(sessionID)
			seed := otReceiver.GetSeed()

			mulReceiver, _ := InitReceiverPhase2(otSender, sessionID, seed, encProofs, nonce)
			mulSender, _ := InitSenderPhase2(otReceiver, sessionID, correlation, vecR, dlogProof, nonce)
			...
			for i := 0; i < L; i++ {
				...
				expected := dkls23.ScalarMul(senderInput[i], receiverRandom)
				if !dkls23.ScalarEqual(sum, expected) {
					t.Errorf("Run %d: Multiplication output[%d] incorrect", run, i)
				}
			}
		})
	}
