...
\begin{remark}[SRS Security]
  \label{rem:srs}
  The current implementation uses \texttt{unsafekzg.NewSRS}, which generates a
  KZG SRS with \emph{known toxic waste}. This breaks proof soundness in an
  adversarial setting: a party with access to the toxic waste can forge proofs.
  For production deployment, this must be replaced with an SRS from a proper
  trusted setup ceremony or a universal SRS (e.g., the Ethereum KZG ceremony
  for BN254).
\end{remark}
...
For production deployment, the unsafe KZG SRS (Remark~\ref{rem:srs}) must be
replaced with a proper trusted setup ceremony, and the hash-to-curve
construction should be evaluated for the target threat model's timing
requirements.
